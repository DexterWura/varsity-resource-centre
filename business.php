...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
	try {
		...
		$pdo = DB::pdo();
		$stmt = $pdo->prepare('INSERT INTO business_reviews (business_id, user_id, rating, title, body) VALUES (?, ?, ?, ?, ?)');
		$userId = $auth->check() ? $auth->user()->getId() : null;
		$stmt->execute([$id, $userId, $rating, $title !== '' ? $title : null, $body]);
		header('Location: /business.php?id=' . $id . '#reviews');
		exit;
	} catch (\Throwable $e) {
		// swallow and continue to render page with any existing data
	}
}
...
try {
	...
	$ownerStmt->execute([(int)$biz['owner_id']]);
	$owner = $ownerStmt->fetch();
	$agg = $pdo->prepare('SELECT AVG(rating) as avg_rating, COUNT(*) as cnt FROM business_reviews WHERE business_id = ?');
	$agg->execute([$id]);
	$aggRow = $agg->fetch();
	$avg = $aggRow ? (float)$aggRow['avg_rating'] : null;
	$count = $aggRow ? (int)$aggRow['cnt'] : 0;
	$revStmt = $pdo->prepare('SELECT br.*, u.full_name FROM business_reviews br LEFT JOIN users u ON br.user_id = u.id WHERE br.business_id = ? ORDER BY br.created_at DESC LIMIT 20');
	$revStmt->execute([$id]);
...
<article class="mx-auto" style="max-width: 860px;">
	...
	<div class="mt-3 d-flex flex-wrap gap-2">
		...
		<a class="btn btn-outline-primary btn-sm" target="_blank" rel="noopener" href="<?= htmlspecialchars($biz['website']) ?>">Visit website</a>
		<?php endif; ?>
		<?php if (!empty($biz['contact_phone'])): ?>
		<a class="btn btn-outline-secondary btn-sm" href="tel:<?= htmlspecialchars($biz['contact_phone']) ?>"><i class="fa-solid fa-phone me-1"></i><?= htmlspecialchars($biz['contact_phone']) ?></a>
		<?php endif; ?>
		<?php if (!empty($biz['contact_email'])): ?>
		<a class="btn btn-outline-secondary btn-sm" href="mailto:<?= htmlspecialchars($biz['contact_email']) ?>"><i class="fa-regular fa-envelope me-1"></i>Email</a>
		<?php endif; ?>
	</div>
